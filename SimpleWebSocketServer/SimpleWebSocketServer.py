...
def _check_unicode(val):  # 返回是否是字符串是吧
    return isinstance(val, str)

class HTTPRequest(BaseHTTPRequestHandler):
   def __init__(self, request_text):
      if VER >= 3:
          self.rfile = BytesIO(request_text)  # 自己定义的rfile，这么秀的嘛，可以呀
      else:
...
class WebSocket(object):
   ...
   def __init__(self, server, sock, address):
      ...
      self.state = HEADERB1  # 1

      # restrict the size of header and payload for security reasons
      self.maxheader = MAXHEADER  # 65536
      self.maxpayload = MAXPAYLOAD  # 33554432
   ...
   def _handlePacket(self):
      if self.opcode == CLOSE:  # 0x8
         pass
      elif self.opcode == STREAM:  # 0x0
         pass
      elif self.opcode == TEXT:  # 0x1
         pass
      elif self.opcode == BINARY:  # 0x2
         pass
      elif self.opcode == PONG or self.opcode == PING:  # 0xA 0x9
         if len(self.data) > 125:  # data数据的长度不能大于125吗，这是为啥，这里装的是啥
      ...
      if self.opcode == CLOSE:  # 0x8
         ...
         if length == 0:  # 原来为0才是正常的意思呀
            pass
         elif length >= 2:
            status = struct.unpack_from('!H', self.data[:2])[0]  # 返回解码的状态吗
            reason = self.data[2:]
            ...
            if len(reason) > 0:
                try:
                    reason = reason.decode('utf8', errors='strict')  # 协议部分，对获取到的数据进行decode解码
                except:
                    status = 1002
         else:
            status = 1002
      ...
      if self.handshaked is False:
         ...
         if not data:
            ...
            if b'\r\n\r\n' in self.headerbuffer:
               self.request = HTTPRequest(self.headerbuffer)  # 这个request能干什么，我很好奇
   ...
   def _sendMessage(self, fin, opcode, data):
        ...
        b2 = 0
        if fin is False:
           b1 |= 0x80  # 于这玩意或以下就ok了吗，，这
        b1 |= opcode  # 是二进制还是str，这玩意的方式还不太一样
        ...
        self.sendq.append((opcode, payload))  # 统一把数据装到一个sendq里面是吧，这样多线程会不会串呀


   def _parseMessage(self, byte):  # 名字确实取的比我好呀，没的说，我还是decode这种杀掉
      # read in the header
...
class SimpleWebSocketServer(object):
   def __init__(self, host, port, websocketclass, selectInterval = 0.1):
      ...
      self.serversocket.listen(5)  # 这一步我是直接继承的socketserver，是不是不太友好
