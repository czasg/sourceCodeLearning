...
class _localimpl:
    ...
    def get_dict(self):
        ...
        defined."""
        thread = current_thread()  # 通过当前线程的内存id来定位数据
        # print(id(thread), '---------------')
        return self.dicts[id(thread)][1]  # 从字典中获取 以线程内存id为key的值，包含(线程弱引用，本地字典)
...
class local:
    ...
    def __new__(cls, *args, **kw):  # () {}
        ...
        object.__setattr__(self, '_local__impl', impl)  # 针对self进行赋值，这个object只是一个通用的方法
        # We need to create the thread dict in anticipation of
        # __init__ being called, to make sure we don't call it
        # again ourselves.
        impl.create_dict()  # 创建线程字典的意思
        return self

    def __getattribute__(self, name):
        with _patch(self):
            return object.__getattribute__(self, name)  # 对object的__getattribute__操作，还是对里面的__dict__进行操作
